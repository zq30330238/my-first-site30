...
- **部署前必须 `export CLOUDFLARE_API_TOKEN="cfut_CkBSWOSXRgR3AGoseiRHUird7zAjZcn3FLQCOcYY54accafd"`**
...
- [CF Token限流] 7个站点并行部署触发CF rate limit(10429)→Authentication error(10000)→Invalid token(9109)。旧Token `cfut_GhVMSufshQgVJ4omlQhX7meWr3RnnMW7mxTBbMQA58e325a4` 于2026-05-23失效，已切换至 `cfut_CkBSWOSXRgR3AGoseiRHUird7zAjZcn3FLQCOcYY54accafd`（2031到期）。预防: 串行部署间隔5秒，单次不超过3个并行。
